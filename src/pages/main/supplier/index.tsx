...
export default function Medicine({ table, session }: any) {
  ...
  const onRemove = async (e: any) => {
    try {
      ...
      const result = await axios.delete(
        CONFIG.base_url_api + `/supplier?id=${formData?.id}`,
        {
          headers: {
            "bearer-token": "stokinventoryapi",
            "x-partner-code": session?.partner_code,
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold">Data Supplier</h2>
        ...
        {modal?.key == "create" || modal?.key == "update" ? (
          ...
          >
            <h2 className="text-xl font-semibold text-center">
              {modal.key == "create" ? "Tambah" : "Ubah"} Data Supplier
            </h2>
