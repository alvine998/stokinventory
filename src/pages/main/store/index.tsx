...
export async function getServerSideProps(context: any) {
  try {
    ...
    const result = await axios.get(
      CONFIG.base_url_api +
        `/stores?pagination=true&page=${+page - 1 || 0}&size=${+size || 10}&search=${search || ""}`,
      {
        headers: {
...
export default function Medicine({ table, session }: any) {
  ...
  const CustomerColumn: any = [
    ...
    },
    {
      ...
      selector: (row: any) =>
        row?.lat && row?.long ? (
          <a className="text-blue-500" href={"https://maps.google.com"}>Lihat Maps</a>
        ) : (
          "-"
        ),
    },
        ...
        {modal?.key == "create" || modal?.key == "update" ? (
          ...
          >
            <h2 className="text-xl font-semibold text-center">
              {modal.key == "create" ? "Tambah" : "Ubah"} Data Toko
            </h2>
