...
export async function getServerSideProps(context: any) {
    try {
        ...
        const result = await axios.get(CONFIG.base_url_api + `/diseases/list?page=${+page || 1}&limit=${+limit || 10}&search=${search || ""}`)
        return {
            props: {
                table: result?.data || []
            }
...
export default function Medicine({ table }: any) {
    ...
    const onSubmit = async (e: any) => {
        ...
        try {
            ...
            if (formData?.id) {
                const result = await axios.patch(CONFIG.base_url_api + `/diseases/update/${formData?.id}`, payload)
            } else {
                const result = await axios.post(CONFIG.base_url_api + `/diseases/create`, payload)
            }
            Swal.fire({
                icon: "success",
    ...
    const onRemove = async (e: any) => {
        try {
            e?.preventDefault();
            const formData = Object.fromEntries(new FormData(e.target))
            const result = await axios.delete(CONFIG.base_url_api + `/diseases/delete/${formData?.id}`)
            Swal.fire({
                icon: "success",
                ...
                </div>
                {
                    modal?.key == "create" || modal?.key == "update" ? <Modal open={modal.open} setOpen={() => setModal({ ...modal, open: false })}>
                        ...
                        <form onSubmit={onSubmit}>
                            ...
                            <Input label='Nama Penyakit' placeholder='Masukkan Nama Penyakit' name='name' defaultValue={modal?.data?.name || ""} required />
